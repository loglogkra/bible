...
public class Stage02BibleTest {
	...
	public void testGetNumberOfVerses() {
		...
		assertEquals("There should be 157 verses, but you have " + numVerses, 157, numVerses);
	}
	...
	public void testGetVerseText() {
		for (Verse expectedVerse : sampleVerses) {
			...
			assertEquals("This verse contains the incorrect text", expectedVerse.getText(), actual);
		}
	}
	...
	public void testGetTitle() {
		assertEquals("The KJV", testBible.getTitle());
	}
	...
	public void testGetVerse() {
		for (Verse expectedVerse : sampleVerses) {
			Reference expectedReference = expectedVerse.getReference();
			// Get the actual verse using the first version of getVerse.
			Verse actualVerse = testBible.getVerse(expectedReference);
			// Get the actual verse using the second version of getVerse.
			Verse actualVerse2 = testBible.getVerse(expectedReference.getBookOfBible(), expectedReference.getChapter(),
					expectedReference.getVerse());

			// For this one I will test the reference and text separately so we know more specifically where it went
			// wrong.
			assertEquals("This verse contains the incorrect text", expectedVerse.getText(), actualVerse.getText());
			//assertEquals("References are different", expectedReference, actualVerse.getReference());

			// For the second version, I will test the Verse object itself. The result should be the same.
			assertEquals(expectedVerse, actualVerse2);
		}
	}

	@Before
	public void setUp() throws Exception {
		sampleVerses = new VerseList("KJV", "The KJV");
		...
		testBible = BibleFactory.createBible(sampleVerses);
	}
}
